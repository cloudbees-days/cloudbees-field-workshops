...
1. Navigate to your workshop GitHub Organization click on the link for the **dev-controller** repository. ![dev-controller repo link](github-dev-controller-repo-link.png?width=50pc)
...
- name: Managers
  ...
  - name: manage-jobs
    grantedAt: child
```
3. Name the new file `rbac.yaml`, copy the `rbac` configuration from above and paste it into the GitHub file editor. Then select the option to **"Create a new branch for this commit and start a pull request"**, name the branch `add-rbac` and then click the **Propose changes** button. ![Create rbac.yaml](github-commit-rbac-yaml.png?width=50pc)
