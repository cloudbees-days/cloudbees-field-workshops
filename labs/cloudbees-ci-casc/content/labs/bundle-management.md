...
3. On the next screen name the new file `update-configuration-bundle` and enter the following contents:
```groovy
...
13. For the **Script Path** replace `Jenkinsfile` with `update-configuration-bundle`. All of the rest of the default values are fine, so click the **Save** button.
